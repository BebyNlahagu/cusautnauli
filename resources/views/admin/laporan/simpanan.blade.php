                ...
                <div class="card-header d-flex align-items-center flex-wrap">
                    ...
                    @if (auth()->user()->role == 'Admin')
                        <div class="btn-group ms-auto">
                            <button type="button" class="btn btn-label-info btn-round btn-sm me-2"
                                data-bs-toggle="dropdown" aria-expanded="false" title="Filter">
                            ...
                            <div class="dropdown-menu p-4" style="min-width: 300px;">
                                <form action="{{ route('laporan.simpanan') }}" method="GET">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="date" name="hari" id="hari" value="{{ request('hari') }}"
                                            class="form-control">
                                    </div>
                            ...
                            <tbody>
                                ...
                                @foreach ($groupedSimpanan as $item)
                                    <tr>
                                        ...
                                        <td class="text-end bold">Rp
                                            {{ number_format($item['total_simpanan'], 0, ',', '.') }}</td>
                                        <td>
                                            <button type="button" class="btn btn-link btn-success view-simpanan-btn"
                                                title="Detail" data-user="{{ $item['user']->name }}"
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.view-simpanan-btn', function() {
                ...
                // filter simpanan berdasarkan user_id
                const userSimpanan = allSimpanan.filter(s => s.user_id == userId);

                let rows = '';
                if (userSimpanan.length > 0) {
                    userSimpanan.forEach((simpan, index) => {
                        rows += `<tr>
                        <td>${index + 1}</td>
                        <td>${simpan.tanggal_terakhir}</td>
                        <td>${simpan.jenis_simpanan}</td>
                        <td class="text-end">Rp ${parseInt(simpan.jumlah_simpanan).toLocaleString('id-ID')}</td>
                    </tr>`;
                    });
                } else {
                ...
                let modalEl = document.getElementById('simpananModal');
