...
<script>
    $(document).ready(function() {
        // Ketika memilih nasabah dari dropdown
        $('#nasabah_id').on('change', function() {
            var nasabah_id = $(this).val();
            
            if (nasabah_id) {
                // Memanggil API untuk mengecek apakah nasabah sudah bergabung lebih dari 6 bulan
                $.ajax({
                    url: '/check-nasabah-bergabung/' + nasabah_id,
                    type: 'GET',
                    success: function(response) {
                        if (response.selisih_bulan < 6) {
                            // Jika nasabah belum bergabung lebih dari 6 bulan, tampilkan pop-up
                ...
                <div class="modal-body">
                    ...
                    <div class="form-floating form-floating-custom mb-3">
                        <select name="lama_pinjaman" id="lama_pinjaman" class="form-control form-select">
                            <option value="">--pilih--</option>
                            <option value="5 Bulan" {{ $n->lama_pinjaman == '5 Bulan' ? 'selected' : '' }}>5 Bulan</option>
                            <option value="10 Bulan" {{ $n->lama_pinjaman == '10 Bulan' ? 'selected' : '' }}>10 Bulan</option>
                            <option value="15 Bulan" {{ $n->lama_pinjaman == '15 Bulan' ? 'selected' : '' }}>15 Bulan</option>
                            <option value="20 Bulan" {{ $n->lama_pinjaman == '20 Bulan' ? 'selected' : '' }}>20 Bulan</option>
                            <option value="25 Bulan" {{ $n->lama_pinjaman == '25 Bulan' ? 'selected' : '' }}>25 Bulan</option>
                            <option value="30 Bulan" {{ $n->lama_pinjaman == '30 Bulan' ? 'selected' : '' }}>30 Bulan</option>
                        </select>
                        <label for="lama_pinjaman">Lama Pinjaman</label>
                    </div>
                    ...
                    {{-- <div class="form-floating form-floating-custom mb-3">
                        <input type="number" name="adm" class="form-control @error('adm') is-invalid @enderror" id="adm" placeholder="Administrasi" value="{{ old('adm', $n->adm) }}" />
...
<script>
    $(document).ready(function() {
        $('#nasabah_id').on('change', function() {
            ...
            if (nasabah_id) {
                // Memanggil API untuk mendapatkan max_pinjaman berdasarkan nasabah_id
                $.ajax({
                    url: '/get-max-pinjaman/' + nasabah_id,
                    type: 'GET',
                    success: function(response) {
                        if (response.max_pinjaman) {
                            // Set nilai max_pinjaman ke inputan jumlah_pinjaman
                            $('#jumlah_pinjaman_display').val('Rp ' + response.max_pinjaman.toLocaleString());
                            $('#jumlah_pinjaman').val(response.max_pinjaman);
                        } else {
                            alert('Max pinjaman tidak ditemukan.');
                        }
                    },
                    error: function() {
                        alert('Terjadi kesalahan saat memuat data max pinjaman.');
                    }
                });
            }
        });
...
<script>
    ...
    function formatUang(input) {
        ...
        document.getElementById('jumlah_pinjaman').value = value;
    }

    document.addEventListener('DOMContentLoaded', function() {
        const displayInput = document.getElementById('jumlah_pinjaman_display');
        const hiddenInput = document.getElementById('jumlah_pinjaman');
    ...
    function formatUangEdit(input, id) {
        ...
        input.value = formatted;
        document.getElementById('jumlah_pinjaman_edit_' + id).value = value;
    }
    
    $(document).ready(function() {

        $('#lama_pinjaman').on('change', function () {
        var bungaPerBulan = 3;

        $('#bunga_pinjaman').val(bungaPerBulan); 
    });
